  ...
  <header className="sticky top-0 z-50 h-20 bg-white border-b border-gray-200 px-8">
    <nav className="flex items-center justify-between h-full max-w-6xl mx-auto">
      ...
      <div className="hidden md:flex items-center gap-8">
        {NAVIGATION_LINKS.map((link) => (
          <Link 
            key={link.href}
            href={link.href} 
            className="text-sm font-medium text-gray-700 hover:text-gray-900 px-4 py-2 rounded-md transition-all duration-200 hover:bg-gray-100"
          >
            {link.label}
          </Link>
        ))}
      ...
      <Link href="/auth/signin">
        <Button className="bg-gray-900 hover:bg-gray-800 text-white border-none rounded-lg px-6 py-3 text-sm font-semibold cursor-pointer transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
          Masuk
        </Button>
      </Link>
    </nav>
...
const HeroSection = () => (
  <section className="py-24 px-8 bg-white flex items-center justify-center min-h-[600px] max-w-6xl mx-auto">
    <div className="text-center max-w-4xl">
      <div className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-700 mb-6">
      ...
      <div className="flex justify-center">
        <Link href="/auth/signin">
          <Button className="bg-gray-900 hover:bg-gray-800 text-white border-none rounded-lg px-8 py-4 text-lg font-semibold cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:shadow-xl">
            Mulai Sekarang
          </Button>
        </Link>
      </div>
...
const FeatureCard = ({ feature }: { feature: typeof FEATURES[0] }) => (
  <Card className="bg-white rounded-2xl p-8 border border-gray-100 min-h-[200px] flex flex-col transition-all duration-300 hover:border-gray-400 hover:-translate-y-2 hover:shadow-lg">
...
const ProcessStep = ({ step }: { step: typeof PROCESS_STEPS[0] }) => (
  <div className="text-center p-8 relative">
    <div className="w-12 h-12 rounded-full bg-gray-900 text-white flex items-center justify-center text-lg font-semibold mx-auto mb-4">
      {step.number}
            </div>
