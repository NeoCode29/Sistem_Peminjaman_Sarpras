...
//     prodiId: z.string().min(1, "Prodi is required"),
// });

// const pegawaiSchema = z.object({
//     nomer_induk: z.string().min(1, "Nomor Induk is required"),
...
export const ProfileService = {
    ...
    getProfile: async (userId: string): Promise<Profile | null> => {
        ...
        if (user.position === "mahasiswa" && user.mahasiswa) {
            return {
                id: user.id,
                name: user.name || "",
                email: user.email || "",
        ...
        } else if (user.position === "pegawai" && user.pegawai) {
            return {
                id: user.id,
                name: user.name || "",
                email: user.email || "",
        ...
        if (profile.posisi === "mahasiswa" && profile.mahasiswa) {
            await prisma.mahasiswa.create({
                data: {
                    ...
                    jurusanId: profile.mahasiswa.jurusanId,
                    prodiId: profile.mahasiswa.prodiId
                }
            });
            ...
            await prisma.pegawai.create({
                data: {
                    userId: profile.id,
                    nomer_induk: profile.pegawai.nomer_induk,
