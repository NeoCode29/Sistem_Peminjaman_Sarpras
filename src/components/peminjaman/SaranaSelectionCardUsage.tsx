...
interface SaranaSelectionProps {
...
/**
 * Enhanced Sarana Selection Component
 * 
 * This component replaces the old dropdown-based selection with:
 ...
 * - Improved UX
 */
export function SaranaSelectionCard({
  availableSarana,
  selectedSarana,
  onAddSarana,
  onRemoveSarana,
  ...
  const hasStockIssues = selectedSarana.some(item => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="max-h-80 overflow-y-auto border rounded-lg bg-muted/20">
            ...
            <div className="grid gap-3 p-3">
              {filteredSarana.map((sarana) => (
                <Card 
                  ...
                  className={cn(
                    "p-3 transition-all duration-200 cursor-pointer border-dashed",
                    "hover:border-solid hover:shadow-sm hover:bg-muted/50"
                  )}
                  onClick={() => {
                    onAddSarana(sarana);
                    setSearchQuery("");
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-medium text-sm truncate">{sarana.nama}</h4>
                        <Badge 
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <span>Jenis: {sarana.jenis}</span>
                        <span>•</span>
                        <span>Status: {sarana.status}</span>
                      </div>
                    </div>
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0 shrink-0 ml-2">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3">
            {selectedSarana.map((item, index) => {
              ...
              return (
                <Card 
                  key={`selected-sarana-${item.id}-${index}`} 
                  className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isOverLimit && (
                        <p className="text-xs text-destructive mt-2 flex items-center gap-1">
                          ...
                          Jumlah yang diminta melebihi stok yang tersedia ({saranaDetail?.sisa} tersisa)
                        </p>
                      )}
                    </div>
