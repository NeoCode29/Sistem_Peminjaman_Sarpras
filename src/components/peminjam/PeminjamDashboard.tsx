...
import { CalendarIcon, Clock, MapPin, Users, ChevronLeft, ChevronRight, FileText, Package } from "lucide-react";
...
export function PeminjamDashboard({ userId }: PeminjamDashboardProps) {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      setError(null);
      try {
        // Fetch all accepted events for public calendar view
  ...
  const getEventColor = (status: StatusPeminjaman, statusPengajuan: StatusPengajuan) => {
    if (statusPengajuan === "PENGAJUAN_DITOLAK" || status === "DIBATALKAN") {
      return "bg-red-500";
  ...
  // Get events that have dates (for calendar highlighting)
                      ...
                      <Card 
                        ...
                        style={{ 
                          borderLeftColor: event.resource.status_peminjaman === "SELESAI" ? "#22c55e" : 
                                          event.resource.status_peminjaman === "DITERIMA" ? "#3b82f6" : "#6b7280" 
                        }}
                        onClick={() => window.open(`/peminjam/peminjaman/${event.resource.id}`, '_blank')}
                      >
